...
v5 request Pulsar exposes today to configure it; the output exists only
as scaffolding for a future capability (wiring is issue #117).

Until it is wired, `StartReplayBuffer` **fails** with `OutputNotRunning`
and the cause libobs recorded, instead of reporting success on a buffer
that never started (issue #120).
