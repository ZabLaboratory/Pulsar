...
| `PULSAR_TRACE_SIGNALS` | `pulsar-frontend-stub` | `all` | Boot-fixed telemetry selector: `all`, `none`, or a CSV of `program`, `preview`, `raw`, `borrowed`, `gpu`, `queues`. Empty, unknown, duplicate, or mixed `all`/`none` values reject telemetry fail-closed. CSV resource samples contain only the selected signal payloads; `all` preserves the composite resource shape for compatibility. The session `capture_paths` array is derived from selected boundaries (`program` → encoder input/encoded packet, `raw` → DirectShow return, `all` → decoded/antenna in addition), so selectors without a boundary signal may legitimately emit an empty array. Take lifecycle, frame identity/PTS, and integrity failures remain mandatory with `none`. |
