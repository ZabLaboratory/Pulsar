...
| `PULSAR_NVENC_LOW_LATENCY` | `pulsar-frontend-stub` | unset / disabled | Explicit NVENC live-latency tradeoff. Only a truthy value (`1`, `true`, `on`, `yes`) selects `tune=ull`, disables multipass/lookahead and sets B-frames to zero. Unset, false, empty or malformed values preserve the historical quality-oriented NVENC defaults; malformed values are logged. This switch does not alter audio routing. |
