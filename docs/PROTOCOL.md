...
| `GetCapabilities` | **Capability manifest** — the authoritative statement of what this Pulsar can do (Prism ADR 027 §3.1/§3.2). Enumerates the encoder families this build exposes (via `obs_enum_encoder_types()`, mapped to Pulsar short names), the bitrate windows, the audio block (monitoring, tracks, sample rate, speaker layout), the presence-only inventories (registered filters, source kinds, destination kinds) and the effective video colorimetry — each with its application regime. Off-air detection; `active_encoder` is the family bound to the streaming output. See the manifest and list-encoding notes below. | — | `version: number`, `capabilities: { [name]: CapabilityEntry }`, `encoders: {value: string}[]`, `active_encoder: string`, `video_bitrate?: {min, max}`, `audio_bitrate?: {value: number}[]`, `error?` |
