...
| `PULSAR_TRACE_SIGNALS` | `pulsar-frontend-stub` | `all` | Boot-fixed telemetry selector: `all`, `none`, or a CSV of `program`, `preview`, `raw`, `borrowed`, `gpu`, `queues`. Empty, unknown, duplicate, or mixed `all`/`none` values reject telemetry fail-closed. Take lifecycle, frame identity/PTS, and integrity failures remain mandatory with `none`. |
