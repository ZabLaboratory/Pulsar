...
def summarize(directory, codec):
    ...
    commits = sorted((r["event"] for r in records if r.get("record_type") == "event"
                      and r["event"].get("event_type") == "TakeCommitted"),
                     key=lambda event: (event["observed_at_monotonic_ns"], event["server_seq"]))
    measured_ids = {event["take_command_id"] for event in commits[int(records[0]["warmup_takes"]):]}
    observations = [r for r in records if r.get("record_type") == "observation"
                    and r["take_command_id"] in measured_ids]
    ...
    content_gap = stats([(r["packet_content_pts_monotonic_ns"] - r["packet_cts_monotonic_ns"]) / 1e6
                         for r in encoded if "packet_content_pts_monotonic_ns" in r])
    ...
    return {
        ...
        "limitations": ["Decoded candidate is a selected picture; marker boundary certifies only the first changed probe marker, neither certifies display.",
...
if __name__ == "__main__":
    ...
    results = [summarize(directory, codec) for directory in args.runs for codec in ("x264", "nvenc")
               if (directory / f"{codec}-report.json").exists()]
    args.output.write_text(json.dumps({"schema": "pulsar.issue253.study.v1", "runs": results}, indent=2) + "\n", encoding="utf-8")
    print("run,codec,raw_p95,dshow_p95,encoded_p95,decoded_candidate_p95,first_marker_p95,rtmp_callback_receiver_p95")
    for r in results:
        latency = r["latency"]
        print(",".join(map(str, [r["run"], r["codec"], *[
            latency[k]["p95_ms"] for k in ("encoder_input_raw", "directshow_return", "encoded_first_packet")
        ], latency.get("decoded_candidate_frame", latency["decoded_first_frame"])["p95_ms"],
            latency.get("decoded_marker_first_frame", {}).get("p95_ms"), r["ac12a"]["p95_ms"]])))
