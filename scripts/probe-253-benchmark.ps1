...
try {
    Get-FileHash -Algorithm SHA256 -LiteralPath $Exe | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $output 'binary.json')
    foreach ($codec in $Encoders) {
        ...
        Write-Host "$codec analyzer exit=$LASTEXITCODE (capacity reference intentionally absent; inspect individual latency gates)"
    }
