...
static void publish(uint64_t frame)
{
    ...
    metadata.valid = 1;
    PULSAR_CHECK(video_queue_write_ex(writer, planes, stride, metadata.pts_ns,
...
int main(void)
{
    ...
    PULSAR_CHECK(reader);
    PULSAR_CHECK(!video_queue_has_unread_frame(NULL));
    PULSAR_CHECK(!video_queue_has_unread_frame(reader));
    publish(1);
    PULSAR_CHECK(video_queue_state(reader) == SHARED_QUEUE_STATE_READY);
    PULSAR_CHECK(video_queue_has_unread_frame(reader));
    ...
    uint64_t pts = 0;
    struct video_queue_frame_metadata metadata = {0};
    PULSAR_CHECK(video_queue_read_ex(reader, &scaler, output, &pts, &metadata));
    PULSAR_CHECK(metadata.frame_id == 1 && pts == 16666667);
    PULSAR_CHECK(!video_queue_has_unread_frame(reader));
    producer_done = CreateEventW(NULL, TRUE, FALSE, NULL);
    PULSAR_CHECK(producer_done);
    ...
    while (WaitForSingleObject(producer_done, 0) != WAIT_OBJECT_0 || video_queue_has_unread_frame(reader)) {
        PULSAR_CHECK((LONG)(deadline - GetTickCount()) > 0);
        if (video_queue_has_unread_frame(reader) && video_queue_read_ex(reader, &scaler, output, &pts, &metadata)) {
            PULSAR_CHECK(metadata.frame_id > last);
            PULSAR_CHECK(pts == metadata.frame_id * 16666667);
            for (unsigned i = 0; i < SIZE; ++i)
                PULSAR_CHECK(output[i] == (uint8_t)(metadata.frame_id % 251));
    ...
    PULSAR_CHECK(WaitForSingleObject(thread, 1000) == WAIT_OBJECT_0);
    PULSAR_CHECK(last == 1000 && samples > 0);
    PULSAR_CHECK(!video_queue_has_unread_frame(reader));
