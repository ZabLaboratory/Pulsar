...
static void BrowserInit(void)
{
	...
	// in our fork (CMakeLists.txt OUTPUT_NAME). The upstream
	// obs-browser-page.exe is removed from the rundir at build time
	// so there is no ambiguity at runtime.
...
// PULSAR: dropped the `EXPORT` qualifier so this stays an internal
// helper. Upstream OBS Studio's frontend calls `obs_browser_initialize`
// via GetProcAddress to bring up browser panels; we have no frontend,
// no caller outside the plugin, and an exported FFI surface here would
// violate LICENSE-INVARIANTS.md (#3) -- caught at the binary level by
// `scripts/check-binary-exports.ps1` and at the source level by
// `scripts/check-license-isolation.sh`. The intra-module call at the
// bottom of this file (BrowserSource::create) still resolves through
// the shared TU.
