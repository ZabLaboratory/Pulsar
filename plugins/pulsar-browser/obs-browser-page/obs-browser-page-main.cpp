...
// PULSAR: upstream obs-browser-page-main.cpp exports NvOptimusEnablement
// and AmdPowerXpressRequestHighPerformance via the Windows dllexport
// qualifier so Windows binds the discrete GPU on switchable-graphics
// laptops. We drop those exports because LICENSE-INVARIANTS.md forbids
// any DLL export under plugins/ (the CI source-grep gate enforces it
// -- see scripts/check-license-isolation.sh). Letting Windows pick the
// GPU is fine for our headless service model -- nothing visible depends
// on which adapter CEF lands on.
